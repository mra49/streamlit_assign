...
def Value_Age(df):
    ...
    st.write("this graph shows us how players market value change with their positions which can hint to the most rare postitions.")
...
def Position_Age(df):
    ...
    st.write("This graph can show us which age group dominates every position")
...
def Value_Position(df):
    ...
    st.write("This graph gives options to explore how several variables affect the market value of players")
...
def main_findings(df):
    ...
    finding1 = '''Finding 1: The best time to sell players is when they are approaching the 28 years mark since this 
    is where their prices soar, the most valuable players aren't the ones that are too young, 
    but the ones who have matured and still are performing at their best (check 1st visual )''' 
    finding2 = '''Finding 2: If players want to increase there career span at the top level,
